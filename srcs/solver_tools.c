...
t_paths_arr		*slv_init_solution(t_map *g)
{
	...
	err = 0;
	if (!(path = ft_memalloc(sizeof(t_paths_arr))))
		return (NULL);
	if (++err && !(path->path = ft_memalloc(sizeof(t_room*) * g->num_nodes)))
		return (slv_error_cleaning(err, path));
	max_path_num = g->start->num_linked_to > g->fin->num_linked_to ?
				   g->start->num_linked_to : g->fin->num_linked_to;
	if (++err && !(path->path_starts = ft_memalloc(sizeof(t_room**) * max_path_num)))
		return (slv_error_cleaning(err, path));
	if (++err && !(path->path_lens = ft_memalloc(sizeof(int) * max_path_num)))
		return (slv_error_cleaning(err, path));
